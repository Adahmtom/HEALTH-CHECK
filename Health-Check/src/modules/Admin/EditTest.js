...
import {  useGetChecklist, useGetSample, useGetTestById, useUpdateTest } from './hooks';
import { errorAlert, infoAlert, successAlert } from '../../utils';
...
const EditTest = () => {
  ...
  const [showChecklistList, setShowChecklistList] = useState(false);
    ...
    const {id} = useParams()
    const {data} = useGetTestById(id)
 
   
    useEffect(()=>{
        setTestName(data?.name)
        setTestPrice(data?.amount)
        setTestSample(data?.sample)
    ...
    const handleAddSample = (sampleName, sampleId) => {
        ...
        if (!exists) {
          ...
          // Handle the case where the sample with the same ID already exists
      ...
      const handleAddChecklist = (checkListName, checkListId) => {
        ...
        const exists = checklist.some((checkList) => checkList._id === checkListId);
  ...
  const removeSample = (sampleId) => {
    ...
    const updatedSamples = testSample.filter((sample) => sample._id !== sampleId);
    setTestSample(updatedSamples);
  };

  const removeChecklist = (ChecklistId) => {
    // Filter out the Checklist with the specified ID
    const updatedChecklists = checklist.filter((CheckList) => CheckList._id !== ChecklistId);
...
addSample(formData);
  ...
  if (isSuccessAdd) {
    ...
    setTestSample([])
    resetAdd();
    successAlert("Test Edited successfully")
     ...
     <div className={styles.testCreatorContainer}>
      ...
      <div className={styles.leftSection}>
        ...
        {checkListSample?.length > 0 &&
          <button onClick={toggleChecklistList} className={styles.showSampleButton}>Checklists</button>}
          {arraySample?.length > 0 && 
          <button onClick={toggleSampleList} className={styles.showSampleButton}> 
            Samples
          </button> }
        </div>
        {showSampleList && (
          <div className={styles.sampleList}>
            ...
            {arraySample?.map((item) => (
              <button className={styles.sampleButton} key={item._id} onClick={() => handleAddSample(item.name, item._id)}>
                {item.name}
              </button>
            ))}
          </div>
         ...
         {showChecklistList && (
          <div className={styles.sampleList}>
            ...
            {checkListSample?.map((item) => (
              <button className={styles.sampleButton} key={item._id} onClick={() => handleAddChecklist(item.name, item._id)}>
                {item.name}
              </button>
            ))}
          </div>
        ...
        <div className={styles.inputContainer}>
          ...
          <div className={styles.sampleButtons}>
            {testSample?.map((sample) => (
              <button key={sample._id} onClick={() => removeSample(sample._id)}>
                {sample.name} <span role="img" aria-label="Remove">❌</span>
              </button>
            ))}
          </div>
        </div>
        <div className={styles.inputContainer}>
          ...
          <div className={styles.sampleButtons}>
            {checklist?.map((sample) => (
              <button key={sample._id} onClick={() => removeChecklist(sample._id)}>
                {sample.name} <span role="img" aria-label="Remove">❌</span>
              </button>
            ))}
          </div>
