...
const CompletedConsult = ({ propId, res }) => {
  ...
  const TotalMe = allAppointments?.filter(
    (appointment) => appointment?.status === "Completed" && appointment?.doctor?.email === appointmentMe
  );
  ...
  const actions = () => [
    {
      name: "View",
      onClick: (res) => {
        navigate(`/app/doctor/completed/consultation/${res._id}`);
      },
    },
  ];
  ...
  return (
    <Layout name="Test" title="Completed Consultations">
